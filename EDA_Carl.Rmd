...
hip_fracture_df = read_sav('/Users/zhixiangyao/Desktop/Spring\ 2023/Intro\ BioMed\ Data\ Sci/Project/BME580_FinalProject/Data/HipFractureQualityData.sav') # Carl's Directory
```
...
sex_race_read <- ggplot(agg_df_srad, aes(x = Sex, fill = Readmission))+
  ...
  facet_wrap(~Race, ncol = 3, nrow = 3, scales = 'free', labeller = label_wrap_gen(25)) +
  labs(title="Readmission of patients with differnt sex and race", x = "Sex", y = "Relative Proportion of Readmission") +
  theme(plot.title = element_text(hjust = 0.5), strip.text = element_text(size=7))
ggsave("readmit_sex+race.png", plot = sex_race_read)
```

```{r}
agg_df_srad30 <- setNames(aggregate(df2$SEX, by=list(df2$SEX,df2$RACE_NEW,df2$ReadmissionPost_117), FUN=length),
                   c("Sex", "Race","Readmissionin30days","Counts"))

sex_race_read30 <- ggplot(agg_df_srad30, aes(x = Sex, fill = Readmissionin30days))+
  geom_bar(mapping = aes(x = Sex, y = Counts, fill = Readmissionin30days), stat = "identity", position = "fill")+
  facet_wrap(~Race, ncol = 3, nrow = 3, scales = 'free', labeller = label_wrap_gen(25)) +
  labs(title="30 days Readmission of patients with differnt sex and race", x = "Sex", y = "Relative Proportion of 30 days readmission") +
  theme(plot.title = element_text(hjust = 0.5), strip.text = element_text(size=7))
ggsave("readmit30_sex+race.png", plot = sex_race_read30)
```

```{r}
agg_df_rad <- setNames(aggregate(df2$RACE_NEW, by=list(df2$RACE_NEW,df2$READMISSION1), FUN=length),
                   c("Race","Readmission","Counts"))

race_read <- ggplot(agg_df_rad, aes(x = Race, fill = Readmission))+
  geom_bar(mapping = aes(x = Race, y = Counts, fill = Readmission), stat = "identity", position = "fill")+
  labs(title="Readmissions of patients with differnt sex and age", x = "Race", y = "Relative Proportion of Readmissions") +
  theme(plot.title = element_text(hjust = 0.5), strip.text = element_text(size=7)) +
  theme(axis.text.x = element_text(angle = -30, hjust = 0.5))
ggsave("readmit_race.png", plot = race_read)
```
